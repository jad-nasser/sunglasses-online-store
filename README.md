...
This project is sunglasses online shopping website that it is built using **MERN** stack web development (**M**ongoDB-**E**xpressJS-**R**eactJS-**N**odeJS). The server side use REST API strategy built with NodeJs, and ExpressJs framework, and MongDB database. The client is built using ReactJS framework.
